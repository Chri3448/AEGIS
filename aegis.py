...
The aegis class is used to generate simulations of photon maps.
'''
...
class aegis():
    ...
    def draw_from_2D_pdf(self, pdf, Ndraws = 0):
        ...
        y_cdfs = np.cumsum(pdf, axis = 1)/(np.tile(np.sum(pdf, axis = 1), (np.size(pdf[0,:]),1)).T + 1e-15) #1e-15 for divide by zero error
    ...
    def draw_spherical_positions_independent(self, input_params, R, Theta, Phi, grains = 1000):
        ...
        r_i = self.draw_from_pdf(r, r_integral/(np.sum(r_integral) + 1e-15), N_draws)
    ...
    def get_counts_histogram_from_roi_map(self, roi_map, mincount, maxcount, N_countbins, countbinspace = 'custom'):
        ...
        countbins[-1] += 1 #ensures the last bin uncludes pixels with the max count
